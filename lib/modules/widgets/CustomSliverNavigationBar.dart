...
/// to also has a [CupertinoNavigationBar] or a [CustomSliverNavigationBar]
...
///
/// By default, only one [CupertinoNavigationBar] or [CustomSliverNavigationBar]
...
///  * [CustomSliverNavigationBar] for a navigation bar to be placed in a
...
class CupertinoNavigationBar extends StatefulWidget implements ObstructingPreferredSizeWidget {
  ...
  /// [CustomSliverNavigationBar] instances of the same [Navigator]. With the
...
///
/// The [CustomSliverNavigationBar] must be placed in a sliver group such
...
/// behavior for multiple navigation bars per route.
///
/// `CustomSliverNavigationBar` has its text scale factor set to 1.0 by default
...
/// `CustomSliverNavigationBar`'s components inside a [MediaQuery] with the
...
///
/// {@tool dartpad}
/// This example shows [CustomSliverNavigationBar] in action inside a [CustomScrollView].
///
...
class CustomSliverNavigationBar extends StatefulWidget {
  ...
  @override
  State<CustomSliverNavigationBar> createState() => _CustomSliverNavigationBarState();
}

// A state class exists for the nav bar so that the keys of its sub-components
// don't change when rebuilding the nav bar, causing the sub-components to
// lose their own states.
class _CustomSliverNavigationBarState extends State<CustomSliverNavigationBar> {
  late _NavigationBarStaticComponentsKeys keys;
...
class _LargeTitleNavigationBarSliverDelegate extends SliverPersistentHeaderDelegate with DiagnosticableTreeMixin {
  ...
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) {
    ...
    return Hero(
      ...
      // CustomSliverNavigationBar like CupertinoNavigationBar because it
...
class _PersistentNavigationBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    Widget? userMiddle = components.middle?.child;
    Widget? userLargeTitle = components.largeTitle?.child;

    Widget? middle;
    if (userMiddle != null && userLargeTitle != null && middleVisible != null) {
          ...
          child: Semantics(
              ...
              child: KeyedSubtree(
                ...
                child: Stack(
                  ...
                  children: [
                    AnimatedOpacity(
                        opacity: !middleVisible! ? 1.0 : 0.0, duration: _kNavBarTitleFadeDuration, child: userMiddle),
                    AnimatedOpacity(
                        opacity: middleVisible! ? 1.0 : 0.0,
                        duration: _kNavBarTitleFadeDuration,
                        child: userLargeTitle),
                  ],
                ),
              )));
    } else if (userMiddle != null) {
      middle = DefaultTextStyle(
        style: CupertinoTheme.of(context).textTheme.navTitleTextStyle,
        child: Semantics(header: true, child: userMiddle),
      );
...
// CustomSliverNavigationBar. The KeyedSubtrees are inserted into static
...
/// [CustomSliverNavigationBar]'s `leading` slot when
/// `automaticallyImplyLeading` is true.
///
